      ...
      <div class="event-hero">
        ...
        <div class="hero-content container">
          ...
          <div class="event-meta">
            ...
            <span class="meta-pill">👥 {{ eventData.participants }} {{ $t('event_detail.tab_entries') }}</span>
            <span class="meta-pill">🗳️ {{ totalEventVotes }} {{ $t('events.votes_sfx') }}</span>
          </div>
          ...
          <div class="hero-actions">
            <button v-if="eventData.category === 'active'" class="btn-submit-entry" @click="handleOpenModal">
              ...
              <span>{{ hasSubmitted ? $t('event_detail.submit_entry_more') : $t('event_detail.submit_entry_now') }}</span>
            </button>
...
<div v-if="currentTab === 'info'" class="info-layout">
  <div class="info-main">
    <div class="card-box shadow-sm">
      <h3>{{ $t('event_detail.intro_rules') }}</h3>
      <p class="desc-text" style="white-space: pre-line;">{{ eventData.description }}</p>
      
      <h3 class="mt-8">{{ $t('event_detail.participation_rules') }}</h3>
      <ul class="rules-list">
  ...
  <div class="real-reward-display">
    <div class="reward-card-inner">
      ...
      <div class="reward-info">
        <span class="reward-label">{{ $t('event_detail.reward_for_winner') }}</span>
      ...
      <p>{{ $t('event_detail.rank_third') }}</p>
          ...
          <div v-else class="entries-layout">
            ...
            <div v-if="topEntries.length > 0" class="ranking-section">
              <h3 class="section-title">{{ $t('event_detail.ranking_title') }}</h3>
            ...
            <div class="all-entries-section mt-12">
              <h3 class="section-title">{{ $t('event_detail.entries_title', { count: entries.length }) }}</h3>
...
const handleOpenModal = () => {
  if (!authStore.isAuthenticated) return toast.warn(t('event_detail.login_submit'))
...
const onEntrySubmitted = () => {
  ...
  toast.success(t('event_detail.submit_success'))
}
