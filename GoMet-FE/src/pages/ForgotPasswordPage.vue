    ...
    <div class="auth-card">
      ...
      <Transition name="fade-slide" mode="out-in">
        <div v-if="state === 'sent'" key="sent" class="success-pane">
          ...
          <h2 class="s-title">{{ t('auth.forgot_sent_title') }}</h2>
          <p class="s-sub">{{ t('auth.forgot_sent_sub') }}</p>
          <button class="btn-primary btn-full" @click="goBack">← {{ t('auth.forgot_back') }}</button>
        </div>
        ...
        <div v-else key="form" class="form-pane">
          ...
          <h1 class="card-title">{{ t('auth.forgot_title') }}</h1>
          <p class="card-sub">{{ t('auth.forgot_sub') }}</p>

          <form @submit.prevent="handleSubmit" novalidate>
            <div class="field-group">
              <label class="field-label">{{ t('auth.forgot_identifier') }}</label>
              <input
                v-model="identifier"
                type="text"
                class="field-input"
            ...
            >
              ...
              <span>{{ state === 'loading' ? t('auth.forgot_sending') : t('auth.forgot_btn') }}</span>
            </button>
          </form>

          <div class="footer-links">
            <button class="link-btn" @click="goBack">← {{ t('auth.forgot_back') }}</button>
          </div>
        </div>
      </Transition>
    </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    errorMsg.value = t('auth.forgot_generic_error')
    state.value = 'idle'
  }
}
