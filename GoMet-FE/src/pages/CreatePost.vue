        ...
        <div class="hero-info-col">
            ...
            <div class="nav-left">
              ...
              <div class="category-badge-selector">
                <select v-model="post.categoryID" class="category-select-clean">
                  <option :value="null">{{ $t('create_post.select_category').toUpperCase() }}</option>
          ...
          <div class="recipe-stats-group">
            <div class="stat-card-clean">
              ...
              <div class="stat-info">
                <span class="s-label">{{ $t('create_post.time_label').toUpperCase() }}</span>
            ...
            <div class="stat-card-clean">
              ...
              <div class="stat-info">
                <span class="s-label">{{ $t('create_post.difficulty_label').toUpperCase() }}</span>
          ...
          <div class="hero-footer-actions">
            <div class="author-info-block">
              <div class="avatar-container">
                <img :src="userAvatar" :alt="$t('common.author_avatar')" @error="handleAvatarError">
              ...
              <div class="auth-text">
                ...
                <span class="auth-name">{{ currentUser.fullName || currentUser.username || $t('compare.default_author') }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="hero-image-col">
          <div class="clean-image-frame uploader" :class="{'has-img': post.image}" @click="triggerUpload">
            ...
            <div v-else class="upload-placeholder">
              ...
              <span>{{ $t('create_post.cover_upload') }}</span>
            </div>
            <div v-if="post.image" class="hover-glass-overlay">{{ $t('create_post.cover_change') }}</div>
            ...
            <div class="video-card-luxury">
              <div class="card-header-dark">
                <div class="h-left">
                  ...
                  <span class="caps-text">{{ $t('create_post.video_optional').toUpperCase() }}</span>
                </div>
              </div>
              <div class="video-aspect-frame uploader-video" @click="!post.video && triggerVideoUpload()">
                <div v-if="!post.video" class="video-placeholder-dark">
                  ...
                  <span>{{ $t('create_post.video_upload') }}</span>
                </div>
                <template v-else>
                  <video :src="post.video" controls playsinline class="html5-player"></video>
                  <button class="btn-remove-media-float" @click.stop="removeVideo">{{ $t('create_post.video_remove') }}</button>
                </template>
          ...
          <div class="process-editorial-section">
            ...
            <div class="process-header">
              ...
              <div class="header-actions">
                <span class="step-count-badge">{{ $t('create_post.steps_count', { count: post.steps.length }).toUpperCase() }}</span>
              </div>
            </div>
                ...
                <div class="timeline-step" v-for="(step, index) in post.steps" :key="step.id">
                  ...
                  <div class="timeline-content">
                    <div class="step-header-row">
                      <h4 class="step-subtitle">{{ $t('create_post.step_label', { count: index + 1 }).toUpperCase() }}</h4>
                      <button class="btn-del-text" @click="removeStep(index)" v-if="post.steps.length > 1">{{ $t('create_post.remove_step') }}</button>
                    </div>
...
const userAvatar = computed(() => {
  ...
  const nameForAvatar = user.fullName || user.username || t('compare.default_author');
...
onMounted(async () => {
  try { categories.value = await getCategories() } catch (e) { toast.error(t('toast.load_categories_failed')) }
})
...
const handleVideoUpload = (e) => {
  const file = e.target.files[0]; if (!file) return;
  if (file.size > 50 * 1024 * 1024) return toast.warn(t('toast.video_limit'));
...
const handlePublish = async () => {
  if (!post.value.title.trim()) return toast.warn(t('toast.create_post_title_required'))
  if (!coverImageFile.value && !post.value.image) return toast.warn(t('toast.create_post_cover_required'))

  const accID = currentUser.value.accountID || currentUser.value.id || currentUser.value.accountId;
  if (!accID) return toast.error(t('toast.session_error'));
  ...
  if (isViolating) {
    toast.error(t('toast.content_policy_post'));
    return;
  }
  ...
  router.push('/home');
  toast.info(`🚀 ${t('toast.post_uploading_bg')}`);
  ...
  (async () => {
    try {
      ...
      if (result?.isApproved === 1 || result?.data?.isApproved === 1) {
        toast.success(`🎉 ${t('toast.post_auto_approved')}`);
      } else {
        toast.success(`✨ ${t('toast.post_pending_approval')}`);
      }

    } catch (err) {
       const msg = `❌ ${t('toast.post_invalid')}`;
       toast.error(msg);
    }
  })();
}
