      ...
      <main class="planner-content">
        <header class="planner-header">
          <div class="header-left">
            <div class="brand-tag">{{ t('mealplan.brand_tag') }}</div>
            <h1 class="main-title">{{ t('mealplan.page_title_prefix') }} <span class="text-serif">{{ t('mealplan.page_title_emphasis') }}</span></h1>
          </div>
          ...
          <div class="header-right">
            ...
            <div class="actions">
              <button class="btn-ai-gen" @click="autoFillAI" :disabled="isLoading" :class="{ loading: isLoading }">
                ...
                <div class="btn-col">
                  <span class="main-text">{{ isLoading ? t('common.loading') : t('mealplan.auto_fill_action') }}</span>
          ...
          <div class="blueprint-grid">
            <div v-for="(day, dIndex) in weekData" :key="dIndex" class="day-column" :class="{ 'today': day.isToday }">
              <div class="col-header">
                <span class="day-name">{{ getDayNameLabel(day.name) }}</span>
                ...
                <div v-for="mealType in ['BREAKFAST', 'LUNCH', 'DINNER']" :key="mealType" class="meal-slot">
                  ...
                  <div v-if="day.meals[mealType]" class="dish-card" @click="showDishDetail(day.meals[mealType])" :class="{'is-completed': day.meals[mealType].isCompleted}">
                    ...
                    <div class="dish-overlay">
                      <span class="dish-cat">{{ day.meals[mealType].category || t('mealplan.custom_meal') }}</span>
                      <h4 class="dish-name">{{ day.meals[mealType].name }}</h4>
                    </div>
                    
                    <button class="btn-check" @click.stop="markCompleted(dIndex, mealType, day.meals[mealType].planId)" :title="t('mealplan.mark_eaten')">
                    ...
                    <button class="btn-remove" @click.stop="removeDish(dIndex, mealType, day.meals[mealType].planId)" :title="t('mealplan.remove_dish')">
...
const currentWeekDisplay = computed(() => {
  ...
  const formatter = new Intl.DateTimeFormat(getDateLocale(locale.value), { day: '2-digit', month: 'short' })
  return `${formatter.format(start)} - ${formatter.format(end)}, ${end.getFullYear()}`
})

const loadWeekData = async () => {
  if (!accountId.value) {
    toast.warn(t('mealplan.login_required'))
    return
  }
  ...
  try {
      ...
      res.data.forEach(plan => {
        if (weekData.value[index].meals[plan.mealType] !== undefined) {
          ...
          weekData.value[index].meals[plan.mealType] = {
            ...
            name: plan.postTitle || plan.customMealName || t('mealplan.dish_fallback', { id: plan.postId }),
    ...
    toast.error(t('mealplan.server_error'))
  } finally { 
...
const removeDish = async (dayIdx, type, id) => {
  if (!confirm(t('mealplan.remove_confirm'))) return
  try {
    await api.delete(`/api/meal-plans/${id}`)
    weekData.value[dayIdx].meals[type] = null
    toast.success(t('mealplan.remove_success'))
  } catch (e) {
    toast.error(t('mealplan.remove_error'))
  }
}
...
const markCompleted = async (dayIdx, type, id) => {
  try {
    ...
    toast.success(t('mealplan.complete_success'))
  } catch (e) {
    toast.error(t('mealplan.complete_error'))
  }
}
...
const showDishDetail = (dish) => {
  if (dish.postId) {
    router.push(`/post/${dish.postId}`)
    return
  }
  toast.info(t('mealplan.custom_dish_toast', { name: dish.name }))
}

const getMealLabel = (mealType) => ({
  BREAKFAST: t('mealplan.meal_breakfast'),
  LUNCH: t('mealplan.meal_lunch'),
  DINNER: t('mealplan.meal_dinner')
})[mealType]

const getDayNameLabel = (dayName) => ({
  MON: t('mealplan.day_mon'),
  TUE: t('mealplan.day_tue'),
  WED: t('mealplan.day_wed'),
  THU: t('mealplan.day_thu'),
  FRI: t('mealplan.day_fri'),
  SAT: t('mealplan.day_sat'),
  SUN: t('mealplan.day_sun')
})[dayName]

onMounted(loadWeekData)
const autoFillAI = () => toast.info(t('mealplan.ai_maintenance'))
</script>
