<template>
  <div class="create-post-wrapper">
    <div v-if="isFetchingData" class="hero-loader" style="height: 100vh; background: #0f172a; display: flex; align-items: center; justify-content: center;">
      <div class="spinner"></div>
      <p style="color: white; margin-left: 20px; font-weight: bold;">Đang tải tuyệt tác...</p>
    </div>
    ...
    <template v-else>
      <section class="hero-section-clean fade-in">
        <div class="hero-container-inner">
          ...
          <div class="hero-image-col">
            <div class="clean-image-frame uploader" :class="{'has-img': post.imagePreview || post.image}" @click="triggerUpload">
              <img v-if="post.imagePreview || post.image" :src="post.imagePreview || post.image" class="img-cover">
              ...
              <div v-if="post.imagePreview || post.image" class="hover-glass-overlay">Thay đổi ảnh bìa</div>
            ...
            <div class="sticky-content-wrapper">
              ...
              <div class="video-card-luxury">
                <div class="card-header-dark">
                  <div class="h-left">
                    ...
                    <span class="caps-text">MASTERCLASS VIDEO</span>
                  </div>
                </div>
                <div class="video-aspect-frame uploader-video" @click="!post.videoPreview && !post.video && triggerVideoUpload()">
                  <div v-if="!post.videoPreview && !post.video" class="video-placeholder-dark">
                  ...
                  <template v-else>
                    <video :src="post.videoPreview || post.video" controls playsinline class="html5-player"></video>
              ...
              <div class="widgets-group">
                <div class="premium-widget ingredients-widget">
                  ...
                  <button class="btn-gradient-orange w-full" @click="addIngredient"><span>+ Thêm nguyên liệu</span></button>
                </div>
              </div>
            ...
            <div class="process-editorial-section">
              ...
              <div class="modern-timeline">
                <transition-group name="list-anim">
                  <div class="timeline-step" v-for="(step, index) in post.steps" :key="step.id || index">
                    ...
                    <div class="timeline-content">
                      ...
                      <div class="step-gallery" @click="triggerStepUpload(index)">
                        <div class="gallery-img-box-lg uploader" :class="{'has-img': step.imagePreview || step.image}">
                          <img v-if="step.imagePreview || step.image" :src="step.imagePreview || step.image">
                          ...
                          <div v-if="step.imagePreview || step.image" class="hover-zoom"><span>Đổi ảnh</span></div>
                        </div>
      ...
      <div class="floating-action-bar-center">
        <div class="fab-inner">
          <button class="btn-cancel-lux" @click="router.back()">Hủy thay đổi</button>
          <button class="btn-publish-lux" @click="handleUpdate">💾 Lưu thay đổi</button>
        </div>
      </div>
    </template>
  </div>
...
// Thêm lại currentUser từ CreatePost để lấy accountID chính xác
...
const post = ref({
  id: null, title: '', description: '', categoryID: null, 
  image: null, imagePreview: null, 
  video: null, videoPreview: null,
  cookingTime: 30, level: 'Medium',
  ingredients: [{ name: '' }],
  steps: [{ id: Date.now(), desc: '', image: null, imagePreview: null }],
})
...
onMounted(async () => {
  if (!postId) return router.push('/home');
  try { 
...
const loadExistingPost = async () => {
  ...
  let parsedIngredients = [{ name: '' }];
  if (data.ingredients) {
    parsedIngredients = data.ingredients.split(',').map(name => ({ name: name.trim() })).filter(i => i.name !== '');
  }

  let parsedSteps = [{ id: Date.now(), desc: '', image: null }];
  if (data.steps && Array.isArray(data.steps)) {
    parsedSteps = data.steps.map((s, idx) => ({
  ...
  post.value = {
    ...
    image: data.image || data.media || null, 
    video: data.video || null, 
    cookingTime: parseInt(data.cookingTime || data.time) || 30,
    level: data.difficulty === 'Easy' ? 'Easy' : (data.difficulty === 'Hard' ? 'Hard' : 'Medium'),
    ingredients: parsedIngredients.length > 0 ? parsedIngredients : [{ name: '' }],
    steps: parsedSteps
  };
}
...
const handleImageUpload = (e) => {
  ...
  coverImageFile.value = file; post.value.imagePreview = createSafeUrl(file);
}
...
const handleVideoUpload = (e) => {
  ...
  videoFile.value = file; post.value.videoPreview = createSafeUrl(file);
}
const removeVideo = () => { videoFile.value = null; post.value.videoPreview = null; post.value.video = null; }

const triggerStepUpload = (idx) => stepInputRefs.value[idx].click()
const handleStepUpload = (e, idx) => {
  const file = e.target.files[0]; if (!file) return;
  stepImageFiles.value[idx] = file; post.value.steps[idx].imagePreview = createSafeUrl(file);
}

const autoResize = (event) => { const el = event.target; el.style.height = 'auto'; el.style.height = el.scrollHeight + 'px'; }
const addIngredient = () => { post.value.ingredients.push({ name: '' }); nextTick(() => { if (ingredientInputs.value?.length) ingredientInputs.value[ingredientInputs.value.length - 1].focus(); }); }
const removeIngredient = (idx) => { if (post.value.ingredients.length > 1) post.value.ingredients.splice(idx, 1) }
const addStep = () => post.value.steps.push({ id: Date.now(), desc: '', image: null, imagePreview: null })
...
const handleUpdate = async () => {
  ...
  if (!post.value.image && !coverImageFile.value) return toast.warn('Vui lòng thêm ảnh bìa!')

  const accID = currentUser.value?.accountID || currentUser.value?.id || authStore.user?.accountID;

  const payloadData = {
    title: post.value.title.trim(),
    description: post.value.description || '',
    categoryID: parseInt(post.value.categoryID) || 1,
  ...
  router.push('/profile'); 
  toast.info('⏳ Đang cập nhật bài viết trong nền...');
  ...
  (async () => {
    try {
      ...
      stepIndices.forEach(idx => uploadTasks.push(uploadMedia(stepImageFiles.value[idx], 'steps')));

      const results = await Promise.all(uploadTasks);
      payloadData.media = results[0]; 
      payloadData.video = results[1]; 

      const uploadedStepUrls = results.slice(2);
      payloadData.steps = payloadData.steps.map((s, i) => {
        const fileIdx = stepIndices.indexOf(i.toString());
        return { desc: s.desc, image: fileIdx !== -1 ? uploadedStepUrls[fileIdx] : s.image }
      });
      
      payloadData.accountID = parseInt(accID);

      await updatePost(postId, payloadData);
      toast.success('🎉 Đã lưu thay đổi tuyệt tác thành công!');
...
@media (max-width: 768px) {
  ...
  .fab-inner { width: 90%; padding: 8px 12px; justify-content: space-between; }
