            ...
            <div class="interaction-col">
              ...
              <div class="review-input-luxury">
                <div class="input-header">
                  <img :src="currentUserAvatar" class="current-user-avt" :alt="$t('common.user_avatar')">
                  
                  <div class="rating-selector" v-if="!hasUserRated">
                    <span class="prompt-text">{{ $t('rating_prompt') }}</span>
                ...
                <div class="textarea-box">
                  ...
                  <div class="input-footer">
                    ...
                    <div class="action-buttons">
                      <button v-if="newComment.trim() || selectedPhotos.length || userRating > 0" class="btn-cancel-review" @click="clearForm" :disabled="isUploading">{{ $t('common.cancel') }}</button>
...
const onPhotosSelected = (e) => {
  if (!authStore.isAuthenticated) { toast.warn(t('toast.need_login_comment')); return }
...
async function loadPost(id) {
  try {
    ...
    if (!canAccess) {
      toast.warn(t('toast.daily_views_exhausted'))
      router.push('/home')
    ...
    post.value = {
      ...
      authorAvatar: dto.authorAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(dto.authorName || t('common.user_fallback'))}&background=EA580C&color=fff`,
      authorBio: dto.authorBio || '', 
      description: dto.description || '',
      time: dto.cookingTime ? `${dto.cookingTime} ${t('common.minutes')}` : '—',
      difficulty: getDifficultyLabel(dto.level),
...
const submitComment = async () => {
  ...
  if (userRating.value > 0 && userRating.value < 3 && !content) {
    toast.warn(t('toast.rating_reason_required'));
    return;
  }
  ...
  if (isViolating) {
    toast.error(t('toast.content_policy_comment'));
  ...
  try {
    ...
    toast.success(t('toast.review_submit_ok'));
  } catch (err) { 
    toast.error(t('toast.review_submit_fail'));
...
const handleSubmitReply = async ({ parentId, content }) => {
  ...
  try {
    ...
    toast.success(t('toast.reply_submit_ok'));
  } catch (err) { 
    toast.error(t('toast.reply_submit_fail'));
  }
}
...
const handleDeleteComment = async (commentId) => {
  ...
  try {
    ...
    if (isAdmin && !isOwnComment) {
      ...
      if (targetComment) {
        targetComment.content = t('toast.comment_hidden_content');
        targetComment.imageUrls = []; // Ẩn luôn hình ảnh vi phạm (nếu có)
      }
      toast.success(t('toast.comment_hidden'));
    } else {
    ...
    toast.error(t('toast.comment_delete_fail')); 
  }
}

const handleToggleLikeComment = async (comment) => {
  if (!authStore.isAuthenticated) { toast.warn(t('toast.need_login_like_comment')); return; }
  const accountID = authStore.user.accountID || authStore.user.id;
  ...
  try {
    ...
    toast.error(t('toast.server_busy_try_later'));
  }
}
