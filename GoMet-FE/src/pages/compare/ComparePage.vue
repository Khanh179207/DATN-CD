  ...
  <div class="compare-page">
    <div class="container">
      ...
      <div class="page-header">
        <h1>{{ t('compare.title') }} ⚖️</h1>
        <button class="btn-back" @click="$router.push('/')">{{ t('compare.back_home') }}</button>
      </div>
      ...
      <div v-if="compareStore.items.length === 0" class="empty-state">
        ...
        <h3>{{ t('compare.empty_title') }}</h3>
        <p>{{ t('compare.empty_desc') }}</p>
        <button class="btn-primary" @click="$router.push('/')">{{ t('compare.explore_now') }}</button>
      </div>

      <div v-else class="compare-grid-wrapper">
        <div class="grid-col labels-col">
          <div class="cell header-cell"></div> <div class="cell label">{{ t('compare.rating') }}</div>
          <div class="cell label">{{ t('compare.author') }}</div>
          <div class="cell label">{{ t('compare.cook_time') }}</div>
          <div class="cell label">{{ t('compare.difficulty') }}</div>
          <div class="cell label">{{ t('compare.calories') }}</div>
          <div class="cell label">{{ t('compare.actions') }}</div>
        </div>
        ...
        >
          <div class="cell header-cell">
            <div class="img-wrap">
              ...
              <button class="btn-remove" @click="compareStore.toggleItem(item)" :title="t('compare.remove')">✕</button>
            </div>
          ...
          <div class="cell" :class="{ 'winner': isBest('likes', item.likes) }">
            <div class="stat-box">
              ...
              <span class="win-badge" v-if="isBest('likes', item.likes)">{{ t('compare.highest') }}</span>
            </div>
          </div>
          ...
          <div class="cell">
            <div class="author-row">
              ...
              <span>{{ item.author?.name || t('compare.default_author') }}</span>
            </div>
          </div>

          <div class="cell" :class="{ 'winner': isBest('time', item.time) }">
            <span class="value">{{ item.time || t('compare.default_time') }}</span>
              <span class="win-badge" v-if="isBest('time', item.time)">{{ t('compare.fastest') }}</span>
          </div>

          <div class="cell">
            <span class="tag-difficulty" :class="getDifficultyClass(item.difficulty)">
              {{ item.difficulty || t('compare.default_difficulty') }}
            </span>
          </div>
          ...
          <div class="cell action-cell">
            <button class="btn-view" @click="goToDetail(item.id)">{{ t('compare.view_recipe') }}</button>
          </div>
        </div>
        ...
        <div v-if="compareStore.items.length < 3" class="grid-col add-col">
          <div class="add-placeholder" @click="$router.push('/')">
            ...
            <span>{{ t('compare.add_another') }}</span>
          </div>
        </div>
