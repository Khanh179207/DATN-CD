    ...
    <div class="main-body-container">
          ...
          <div class="panel-layout">
            ...
            <div class="filter-row-bottom">
              <div class="filter-col reveal-item" style="--delay: 0.2s">
                ...
                <div class="chip-group">
                  <label class="radio-chip"><input type="radio" v-model="filters.difficulty" value=""><span>{{ $t('search.all') }}</span></label>
              ...
              <div class="filter-col reveal-item" style="--delay: 0.3s">
                <h4>{{ $t('search.time') }}</h4>
                <div class="chip-group">
                  <label class="radio-chip"><input type="radio" v-model="filters.time" value=""><span>{{ $t('search.all') }}</span></label>
            ...
            <div class="filter-row-bottom premium-exclusive-row">
              ...
              <div class="filter-col reveal-item" style="--delay: 0.4s">
                <h4><span class="premium-icon">✨</span> {{ $t('search.special_filters') }}</h4>
              ...
              <div class="filter-col reveal-item" style="--delay: 0.5s">
                <h4><span class="premium-icon">🥕</span> {{ $t('search.ingredient_title') }}</h4>
                <div class="ingredient-include-wrapper">
                  <div class="input-with-icon">
                    <input 
                      ...
                      :placeholder="$t('search.ingredient_placeholder')"
                    />
      ...
      <div class="results-grid-container" :class="{ 'is-loading': loading }">
        ...
        <div v-if="paginatedData.length === 0 && !loading" class="empty-state-depth">
          ...
          <h3>{{ currentTab === 'recipes' ? $t('search.no_results_title_recipes') : $t('search.no_results_title_users') }}</h3>
          <p>{{ $t('search.no_results_desc') }}</p>
          <button v-if="currentTab === 'recipes'" class="btn-back-all" @click="resetFilters">{{ $t('search.clear_filters') }}</button>
        </div>
      </div>
...
const toggleAdvancedFilter = () => {
  if (!authStore.isAuthenticated) {
    toast.warn(t('search.premium_login_required'));
    return;
  }
  if (!isPremiumUser.value) {
    toast.warn(t('search.premium_required'));
    return;
  }
...
async function fetchUsersRaw() {
  ...
  try {
    ...
    allUsers.value = rawData.map(user => ({
      ...
      name: user.fullName || user.username || user.name || t('recipe.anonymous_user'),
...
onMounted(async () => {
  ...
  try {
    ...
    categories.value = [
      { id: null, name: t('search.all_categories') }, 
      ...cats.map(c => ({ id: c.categoryID, name: c.categoryName }))
    ]
  } catch (e) {
    categories.value = [{ id: null, name: t('search.all_categories') }]
  }
  if (currentTab.value === 'recipes') fetchPostsRaw()
  else fetchUsersRaw()
})
