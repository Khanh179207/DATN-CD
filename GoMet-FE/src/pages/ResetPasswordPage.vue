  ...
  <div class="auth-page">
    ...
    <div class="auth-card">
      ...
      <div class="brand">
        ...
        <span class="brand-sub">{{ t('auth.brand_subtitle') }}</span>
      </div>
      ...
      <Transition name="fade-slide" mode="out-in">
        ...
        <div v-if="state === 'invalid'" key="invalid" class="result-pane">
          ...
          <h2 class="r-title">{{ t('auth.reset_invalid_title') }}</h2>
          <p class="r-sub">{{ t('auth.reset_invalid_sub') }}</p>
          <button class="btn-primary btn-full" @click="goForgot">{{ t('auth.reset_request_new_link') }}</button>
        </div>
        ...
        <div v-else-if="state === 'success'" key="success" class="result-pane">
          ...
          <h2 class="r-title">{{ t('auth.reset_success_title') }}</h2>
          <p class="r-sub">{{ t('auth.reset_success_sub') }}</p>
          <button class="btn-primary btn-full" @click="goLogin">{{ t('auth.reset_go_login') }}</button>
        </div>
        ...
        <div v-else key="form" class="form-pane">
          ...
          <h1 class="card-title">{{ t('auth.reset_title') }}</h1>
          <p class="card-sub">{{ t('auth.reset_sub') }}</p>
          ...
          <form @submit.prevent="handleReset" novalidate>
            ...
            <div class="field-group">
              <label class="field-label">{{ t('auth.reset_new_password_label') }}</label>
            ...
            <!-- Confirm Password -->
            <div class="field-group">
              <label class="field-label">{{ t('auth.reset_confirm_password_label') }}</label>
              <div class="input-wrap">
                <input
            ...
            >
              ...
              <span>{{ state === 'loading' ? t('auth.reset_updating') : t('auth.reset_button') }}</span>
            </button>
          </form>

          <div class="footer-links">
            <button class="link-btn" @click="goForgot">{{ t('auth.reset_back_request_new') }}</button>
          </div>
        </div>
...
const strengthText  = computed(() => ({
  weak: t('auth.reset_strength_weak'),
  fair: t('auth.reset_strength_fair'),
  good: t('auth.reset_strength_good'),
  strong: t('auth.reset_strength_strong'),
}[strengthLevel.value] || ''))
...
const validateFields = () => {
  ...
  const pw = newPassword.value
  if (!pw || pw.length < 8) {
    fieldErrors.value.newPassword = t('auth.reset_error_min_length')
    ok = false
  } else if (!/[A-Z]/.test(pw)) {
    fieldErrors.value.newPassword = t('auth.reset_error_uppercase')
    ok = false
  } else if (!/[a-z]/.test(pw)) {
    fieldErrors.value.newPassword = t('auth.reset_error_lowercase')
    ok = false
  } else if (!/\d/.test(pw)) {
    fieldErrors.value.newPassword = t('auth.reset_error_number')
    ok = false
  }
  if (pw !== confirmPassword.value) {
    fieldErrors.value.confirmPassword = t('auth.reset_error_mismatch')
    ok = false
  }
  return ok
}
...
const handleReset = async () => {
  ...
  try {
    ...
    if (msg && (msg.toLowerCase().includes('invalid') || msg.toLowerCase().includes('expired') || msg.toLowerCase().includes('used'))) {
      ...
      serverError.value = t('auth.reset_generic_error')
      state.value = 'idle'
    }
  }
}
