        ...
        <div class="receipt-card">
          ...
          <div class="receipt-top">
            <div class="r-logo-wrap">
              ...
              <h3 class="r-title">{{ $t('admin.transactions.success_title') }}</h3>
...
const formatCurrency = (val) => {
  ...
  return `${new Intl.NumberFormat(locale.value === 'vi' ? 'vi-VN' : 'en-US').format(val)} ${t('admin.transactions.currency')}`
}

const formatDate = (dateString) => {
  if (!dateString) return t('admin.common.empty_value')
  return formatLocaleDateTime(dateString, locale.value)
}
...
const exportToExcel = () => {
  ...
  const dataToExport = filteredTransactions.value.map((txn, index) => ({
    [t('admin.transactions.export.index')]: index + 1,
    [t('admin.transactions.export.code')]: txn.orderCode,
    [t('admin.transactions.export.customer')]: txn.username,
    [t('admin.transactions.export.email')]: txn.email,
    [t('admin.transactions.export.plan')]: txn.planName,
    [t('admin.transactions.export.amount')]: txn.amount,
    [t('admin.transactions.export.status')]: getStatusLabel(txn.status),
    [t('admin.transactions.export.created_at')]: formatDate(txn.createdAt),
    [t('admin.transactions.export.paid_at')]: formatDate(txn.paidAt)
  }))
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, t('admin.transactions.export.sheet_name'))
