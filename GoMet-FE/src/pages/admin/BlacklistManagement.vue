    ...
    <div class="add-word-section anim-fade-up">
      <div class="add-card-lux">
        ...
        <button class="btn-lux-add" @click="addWord" :disabled="!newWord.trim() || isAdding">
          ...
          <Plus v-else :size="18" />
          <span>{{ t('admin.blacklist.add_button') }}</span>
        </button>
      </div>
    </div>

    <div v-if="loading" class="empty-state-lux"><Loader2 :size="32" class="spin-icon" /> <p>{{ t('admin.blacklist.loading') }}</p></div>
      ...
      <TransitionGroup name="list-anim">
        <div v-for="item in words" :key="item.wordID" class="word-card-lux">
          <div class="word-content">
            ...
            <small class="word-date">{{ t('admin.blacklist.added_date') }} {{ formatDate(item.createdAt) }}</small>
          </div>
          <button class="btn-remove" @click="removeWord(item.wordID)" :title="t('admin.blacklist.remove_title')">
            <Trash2 :size="16" />
          </button>
        </div>
...
const fetchWords = async () => {
  ...
  try {
    ...
    error.value = t('admin.blacklist.load_failed_prefix').replace(/[:：]\s*$/, '')
  } finally {
    loading.value = false
  }
}
...
const addWord = async () => {
  ...
  try {
    ...
    newWord.value = ''
    showToast(t('admin.blacklist.add_success'))
  } catch (e) {
    showToast(t('admin.blacklist.add_failed'), 'error')
  } finally {
...
const removeWord = async (id) => {
  ...
  try {
    ...
    showToast(t('admin.blacklist.remove_success'))
  } catch (e) {
    showToast(t('admin.blacklist.remove_failed'), 'error')
  }
}
