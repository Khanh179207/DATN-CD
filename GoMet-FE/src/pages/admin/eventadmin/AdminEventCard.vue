<template>
  <div class="event-row-card" :class="{'is-deleted': getStatus(ev) === 'deleted'}">
      ...
      <div class="info-block">
        ...
        <div class="visual-timeline">
          <div class="step" :class="phaseClass(ev.startAt, ev.endAt)">
            <span class="dot"></span>
            <span class="t-label">Nộp bài</span>
          ...
          <div class="step" :class="phaseClass(ev.voteStartAt, ev.voteEndAt)">
            <span class="dot"></span>
            <span class="t-label">Bình chọn</span>
    ...
    <div class="card-stats">
      <div class="stat-pill">
        <span class="s-lbl">Bài dự thi</span>
        <span class="s-val">{{ ev.postCount || 0 }}</span>
      </div>
      <div class="stat-pill">
        <span class="s-lbl">Lượt vote</span>
    ...
    <div class="card-actions">
      ...
      <button class="btn-circle view-posts" @click="$emit('goToPostManagement', ev.eventID)" title="Quản lý Bài dự thi">
...
const getStatusText = (ev) => ({ ended: 'Kết thúc', upcoming: 'Sắp diễn ra', active: 'Đang chạy', deleted: 'Đã Ẩn' }[getStatus(ev)])
const getVisibilityText = (ev) => ev.isActive === 0 ? 'Bị ẩn' : 'Đang hiển thị'
...
const phaseLabel = (startDate, endDate) => {
  ...
  const now = new Date();
  if (now > new Date(endDate)) return '(Đã qua)';
  if (now >= new Date(startDate) && now <= new Date(endDate)) return '(Đang mở)';
  return '(Sắp tới)';
}

const formatDate = (d) => d ? new Date(d).toLocaleString('vi-VN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false }) : '—'
</script>
