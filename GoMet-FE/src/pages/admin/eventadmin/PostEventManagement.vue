  ...
  <div class="pe-admin-wrapper">
    ...
    <div class="pe-top-bar">
      <button class="btn-back-minimal" @click="goBack">
        <i class="fas fa-arrow-left"></i> <span>{{ $t('admin.event_posts.back') }}</span>
      </button>
    ...
    <div class="pe-hero-card" v-if="eventInfo">
      <div class="hero-main">
        <div class="hero-banner">
          <img :src="getImageUrl(eventInfo.bannerImage)" :alt="$t('admin.event_posts.banner_alt')" />
        </div>
        <div class="hero-info">
          <div class="event-id-tag">{{ $t('admin.events.event_id') }}: #{{ eventID }}</div>
          ...
          <div class="event-timeline-grid">
            <div class="timeline-item">
              ...
              <div class="t-content">
                <label>{{ $t('admin.event_posts.submission_phase') }}</label>
            ...
            <div class="timeline-item highlight">
              ...
              <div class="t-content">
                <label>{{ $t('admin.event_posts.voting_phase') }}</label>
      ...
      <div class="hero-stats-panel">
        <div class="stat-unit">
          ...
          <span class="lbl">{{ $t('admin.event_posts.entries') }}</span>
        </div>
        <div class="stat-divider"></div>
        <div class="stat-unit">
          <span class="val text-orange">{{ totalVotes }}</span>
          <span class="lbl">{{ $t('admin.event_posts.total_votes') }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="pe-content-card">
      <div class="content-header">
        <div class="search-wrapper">
          ...
          <input v-model="searchQuery" type="text" :placeholder="$t('admin.event_posts.search_placeholder')" />
        </div>
        
        <div v-if="isEnded" class="locked-badge">
          <i class="fas fa-lock"></i> {{ $t('admin.event_posts.locked') }}
        </div>
      </div>

      <div v-if="loading" class="loading-zone">
        <div class="loader"></div>
        <p>{{ $t('admin.event_posts.loading') }}</p>
      </div>
      ...
      <div v-else-if="posts.length === 0" class="empty-zone">
        ...
        <h3>{{ $t('admin.event_posts.empty_title') }}</h3>
        <p>{{ $t('admin.event_posts.empty_desc') }}</p>
      </div>

      <div v-else class="pe-table-wrapper">
        <div class="pe-thead">
          <div class="th col-rank">{{ $t('admin.event_posts.rank') }}</div>
          <div class="th col-post">{{ $t('admin.event_posts.entry') }}</div>
          <div class="th col-author">{{ $t('admin.event_posts.author') }}</div>
          <div class="th col-vote text-center">{{ $t('admin.event_posts.votes') }}</div>
          <div class="th col-action text-right">{{ $t('admin.event_posts.tools') }}</div>
        </div>
        ...
        <div class="pe-tbody">
          ...
          >
            ...
            <div class="td col-action text-right">
              <button class="btn-tool view" @click="openPostInNewTab(post.postID)" :title="$t('admin.event_posts.view_post')">
              ...
              <button 
                ...
                :title="$t('admin.event_posts.remove_from_event')"
              >
    ...
    <Transition name="zoom-in">
      <div v-if="showKickModal" class="pe-modal-overlay" @click.self="showKickModal = false">
        <div class="pe-modal-content danger">
          ...
          <h2>{{ $t('admin.event_posts.kick_title') }}</h2>
          <p>{{ $t('admin.event_posts.kick_desc', { name: postToKick?.username }) }}</p>
          <div class="modal-warning">
             <i class="fas fa-info-circle"></i> {{ $t('admin.event_posts.kick_note') }}
          </div>
          <div class="modal-btns">
            <button class="btn-cancel" @click="showKickModal = false">{{ $t('common.cancel') }}</button>
            <button class="btn-confirm-danger" @click="executeKick">{{ $t('admin.event_posts.confirm_remove') }}</button>
          </div>
        </div>
      </div>
    </Transition>
...
const getEventStage = (ev) => {
  if (Number(ev.isActive) === 0) return { code: 'deleted', text: t('admin.event_posts.stage_deleted') };
  if (Number(ev.isForceEnded) === 1) return { code: 'ended', text: t('admin.event_posts.stage_ended_early') };
  ...
  if (now < start) return { code: 'upcoming', text: t('admin.event_posts.stage_upcoming') };
  if (now >= start && now <= end) return { code: 'active', text: t('admin.event_posts.stage_submitting') };
  if (now > end && now <= voteEnd) return { code: 'voting', text: t('admin.event_posts.stage_voting') };
  return { code: 'ended', text: t('admin.event_posts.stage_ended') };
};
...
const loadData = async () => {
  ...
  try {
    ...
    toast.error(t('admin.event_posts.load_failed'));
  } finally {
    loading.value = false;
  }
}
...
const executeKick = async () => {
  try {
    ...
    toast.success(t('admin.event_posts.kick_ok'));
    showKickModal.value = false;
  } catch (error) {
    toast.error(t('admin.event_posts.action_failed'));
  }
}
</script>
