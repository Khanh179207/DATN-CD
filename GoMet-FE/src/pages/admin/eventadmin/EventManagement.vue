<template>
  <div class="gomet-admin-pro">
    ...
    <div class="stats-bar">
      ...
      <div class="divider"></div>
      <div class="stat-item"><span class="s-label">Tổng bài thi</span><span class="s-val text-orange">{{ totalPostCount }}</span></div>
    </div>
    ...
    <div v-else-if="error" class="error-banner">⚠️ {{ error }} <button @click="loadEvents">Thử lại</button></div>
        ...
        <div class="modal-form wide-modal">
          <div class="mf-header">
            <div class="mf-header-left">
              ...
              <h3 v-else>{{ isEditing ? '✏️ Chỉnh sửa sự kiện' : '✨ Tạo sự kiện mới' }}</h3>
          ...
          <div class="modal-scroll-area">
            ...
            <div class="form-group">
              <label>Tên sự kiện <span v-if="!isViewOnly" class="req">*</span></label>
              <input v-model="form.eventName" type="text" placeholder="VD: Top Chef Gomet 2026" :disabled="isViewOnly">
            </div>
            <div class="form-group">
              <label>Mô tả sự kiện</label>
              <textarea v-model="form.description" rows="3" placeholder="Giới thiệu về cuộc thi..." :disabled="isViewOnly"></textarea>
            </div>
            <div class="form-group">
            ...
            <div class="form-grid">
               ...
               <div class="form-column">
                  ...
                  <div class="form-group"><label>Bắt đầu nhận bài</label><input v-model="form.startAt" type="datetime-local" :disabled="isViewOnly"></div>
                  <div class="form-group"><label>Kết thúc nhận bài</label><input v-model="form.endAt" type="datetime-local" :disabled="isViewOnly"></div>
                  <div class="form-group"><label>Bắt đầu bình chọn</label><input v-model="form.voteStartAt" type="datetime-local" :disabled="isViewOnly"></div>
                  <div class="form-group"><label>Kết thúc bình chọn</label><input v-model="form.voteEndAt" type="datetime-local" :disabled="isViewOnly"></div>
               </div>
            </div>
          </div>
          ...
          <div class="mf-actions">
            ...
            <div class="mf-actions-right">
              <button @click="showModal = false" class="btn-cancel">{{ isViewOnly ? 'Đóng' : 'Hủy' }}</button>
              <button v-if="!isViewOnly" @click="saveEvent" class="btn-save" :disabled="saving">
                <span v-if="saving">Đang xử lý...</span><span v-else>{{ isEditing ? 'Lưu thay đổi' : 'Tạo ngay' }}</span>
              </button>
            </div>
          </div>
        </div>
    ...
    <Transition name="zoom-in">
      <div v-if="showConfirmEndModal" class="modal-overlay" @click.self="showConfirmEndModal = false">
        <div class="confirm-modal">
          ...
          <p>Bạn đang yêu cầu đóng sự kiện <strong>"{{ eventToClose?.eventName }}"</strong> ngay lập tức.</p>
          ...
          <div class="confirm-actions">
            <button class="btn-gray" @click="showConfirmEndModal = false">Hủy thao tác</button>
            <button class="btn-danger" @click="executeForceEnd">Vẫn đóng sự kiện</button>
          </div>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="zoom-in">
      <div v-if="showDeleteRestoreModal" class="modal-overlay" @click.self="showDeleteRestoreModal = false">
        <div class="confirm-modal">
          ...
          <h2>{{ isActionSoftDelete ? 'Xác nhận ẩn sự kiện' : 'Xác nhận khôi phục' }}</h2>
          <p v-if="isActionSoftDelete">Bạn có chắc muốn ẩn sự kiện <strong>"{{ eventTargetName }}"</strong> khỏi hệ thống?</p>
          <p v-else>Sự kiện <strong>"{{ eventTargetName }}"</strong> sẽ được hiển thị trở lại cho người dùng. Bạn đồng ý chứ?</p>
          
          <div class="confirm-actions mt-4">
            <button class="btn-gray" @click="showDeleteRestoreModal = false">Hủy bỏ</button>
            <button 
    ...
    <Transition name="zoom-in">
      <div v-if="showWinnerModal" class="modal-overlay" @click.self="showWinnerModal = false">
        <div class="winner-modal-card">
          ...
          <div v-if="loadingWinner" class="winner-loading">Đang tính kết quả...</div>
          <div v-else-if="!winnerData" class="winner-empty">Không tìm thấy bài dự thi hợp lệ.</div>
          <div v-else class="winner-content">
            ...
            <p class="winner-author">Tác giả: <strong>{{ winnerData.username }}</strong></p>
            <p class="winner-id">Account ID: <strong>#{{ winnerData.accountID }}</strong></p>
...
const fetchWinnerForEvent = async (event) => {
  ...
  } catch (error) { console.error("Lỗi lấy Winner", error); }
}
...
const executeForceEnd = async () => {
  ...
  try {
    ...
    toast.success('Sự kiện đã được đóng khẩn cấp! ⚡')
    showConfirmEndModal.value = false;
    eventToClose.value = null;
    loadEvents() 
  } catch(e) { toast.error('Lỗi khi kết thúc sự kiện!') }
}
...
const viewWinner = async (ev) => {
  ...
  } catch (e) { toast.error('Lỗi lấy dữ liệu') } finally { loadingWinner.value = false }
}
