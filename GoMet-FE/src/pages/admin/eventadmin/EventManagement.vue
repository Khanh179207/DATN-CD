<template>
  <div class="gomet-admin-pro">
    ...
    <div class="stats-bar">
      <div class="stat-item"><span class="s-label">{{ $t('admin.events.total_events') }}</span><span class="s-val">{{ events?.length || 0 }}</span></div>
      <div class="divider"></div>
      <div class="stat-item"><span class="s-label">{{ $t('admin.common.active') }}</span><span class="s-val text-green">{{ ongoingCount }}</span></div>
      <div class="divider"></div>
      <div class="stat-item"><span class="s-label">{{ $t('admin.events.total_entries') }}</span><span class="s-val text-orange">{{ totalPostCount }}</span></div>
    </div>
    ...
    <div class="toolbar">
      ...
      <div class="filter-group">
        <select v-model="statusFilter">
          ...
          <option value="upcoming">{{ $t('admin.common.upcoming') }}</option>
          <option value="ended">{{ $t('admin.common.ended') }}</option>
          <option value="deleted">{{ $t('admin.common.deleted') }}</option>
        </select>
      </div>
    </div>
    ...
    <div v-else-if="error" class="error-banner">⚠️ {{ error }} <button @click="loadEvents">{{ $t('admin.common.retry') }}</button></div>
    ...
    <div v-else class="event-list-container">
      <AdminEventCard 
        ...
        @deleteEvent="confirmDeleteRestore" 
        @goToPostManagement="goToPostManagement"
      />

      <div v-if="!filteredEvents || filteredEvents.length === 0" class="empty-state">{{ $t('admin.events.empty') }}</div>
    </div>
        ...
        <div class="modal-form wide-modal">
          <div class="mf-header">
            <div class="mf-header-left">
              ...
              <h3 v-else>{{ isEditing ? `✏️ ${$t('admin.events.edit_event')}` : `✨ ${$t('admin.events.create_event')}` }}</h3>
          ...
          <div class="modal-scroll-area">
            ...
            <div class="form-group">
              <label>{{ $t('admin.events.event_name') }} <span v-if="!isViewOnly" class="req">*</span></label>
              <input v-model="form.eventName" type="text" :placeholder="$t('admin.events.event_name_placeholder')" :disabled="isViewOnly">
            </div>
            <div class="form-group">
              <label>{{ $t('admin.events.description') }}</label>
              <textarea v-model="form.description" rows="3" :placeholder="$t('admin.events.description_placeholder')" :disabled="isViewOnly"></textarea>
            </div>
            <div class="form-group">
              <label>{{ $t('admin.events.banner') }}</label>
              <div class="upload-zone" :class="{'disabled-zone': isViewOnly}">
                <input v-if="!isViewOnly" type="file" id="fileUpload" accept="image/*" @change="handleFileUpload" hidden>
                ...
                <div v-else class="preview-container">
                  ...
                  <button v-if="!isViewOnly" class="btn-remove-img" @click="removeImage">✕</button>
                </div>
              </div>
            </div>
            ...
            <div class="form-grid">
               <div class="form-column">
                 ...
                 <div class="form-group">
                    <label>{{ $t('admin.events.max_votes_per_user') }} <span v-if="!isViewOnly" class="req">*</span></label>
                    <input v-model="form.maxVotes" type="number" min="1" :disabled="isViewOnly">
                 </div>
                 <div class="form-group">
                    <label>{{ $t('admin.events.detailed_rules') }}</label>
               ...
               <div class="form-column">
                  ...
                  <div class="form-group"><label>{{ $t('admin.events.submission_start') }}</label><input v-model="form.startAt" type="datetime-local" :disabled="isViewOnly"></div>
                  <div class="form-group"><label>{{ $t('admin.events.submission_end') }}</label><input v-model="form.endAt" type="datetime-local" :disabled="isViewOnly"></div>
                  <div class="form-group"><label>{{ $t('admin.events.voting_start') }}</label><input v-model="form.voteStartAt" type="datetime-local" :disabled="isViewOnly"></div>
                  <div class="form-group"><label>{{ $t('admin.events.voting_end') }}</label><input v-model="form.voteEndAt" type="datetime-local" :disabled="isViewOnly"></div>
               </div>
            </div>
          </div>
          ...
          <div class="mf-actions">
            <div class="mf-actions-left">
              <button
                v-if="isEditing && !isViewOnly && (getStatusHelper(form) === 'active' || getStatusHelper(form) === 'upcoming')"
                class="btn-end-event"
                @click="confirmForceEnd(form)">
                {{ $t('admin.events.end_early') }}
              </button>
            </div>
            <div class="mf-actions-right">
              <button @click="showModal = false" class="btn-cancel">{{ isViewOnly ? $t('admin.common.close') : $t('admin.common.cancel') }}</button>
              <button v-if="!isViewOnly" @click="saveEvent" class="btn-save" :disabled="saving">
                <span v-if="saving">{{ $t('admin.events.processing') }}</span><span v-else>{{ isEditing ? $t('admin.events.save_changes') : $t('admin.events.create_now') }}</span>
              </button>
            </div>
          </div>
        </div>
    ...
    <Transition name="zoom-in">
      <div v-if="showConfirmEndModal" class="modal-overlay" @click.self="showConfirmEndModal = false">
        <div class="confirm-modal">
          ...
          <p>{{ $t('admin.events.force_end_message', { name: eventToClose?.eventName }) }}</p>
          ...
          <div class="confirm-actions">
            <button class="btn-gray" @click="showConfirmEndModal = false">{{ $t('admin.events.cancel_action') }}</button>
            <button class="btn-danger" @click="executeForceEnd">{{ $t('admin.events.force_end_anyway') }}</button>
          </div>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="zoom-in">
      <div v-if="showDeleteRestoreModal" class="modal-overlay" @click.self="showDeleteRestoreModal = false">
        <div class="confirm-modal">
          ...
          <h2>{{ isActionSoftDelete ? $t('admin.events.confirm_hide_title') : $t('admin.events.confirm_restore_title') }}</h2>
          <p v-if="isActionSoftDelete">{{ $t('admin.events.hide_confirm', { name: eventTargetName }) }}</p>
          <p v-else>{{ $t('admin.events.restore_confirm', { name: eventTargetName }) }}</p>
          
          <div class="confirm-actions mt-4">
            <button class="btn-gray" @click="showDeleteRestoreModal = false">{{ $t('admin.common.cancel') }}</button>
            <button 
              :class="isActionSoftDelete ? 'btn-danger' : 'btn-success'" 
              @click="executeDeleteRestore"
            >
    ...
    <Transition name="zoom-in">
      <div v-if="showWinnerModal" class="modal-overlay" @click.self="showWinnerModal = false">
        <div class="winner-modal-card">
          ...
          <div v-if="loadingWinner" class="winner-loading">{{ $t('admin.events.calculating_results') }}</div>
          <div v-else-if="!winnerData" class="winner-empty">{{ $t('admin.events.no_valid_entry') }}</div>
          <div v-else class="winner-content">
            ...
            <p class="winner-author">{{ $t('admin.events.author') }} <strong>{{ winnerData.username }}</strong></p>
            <p class="winner-id">{{ $t('admin.events.account_id') }}: <strong>#{{ winnerData.accountID }}</strong></p>
...
const form = reactive({ 
  ...
  description: '', rules: '', reward: '', maxVotes: 3,
  startAt: '', endAt: '', voteStartAt: '', voteEndAt: '',
  originalEndAt: '', originalVoteEndAt: '',
  winner: null, isActive: 1, isForceEnded: 0
})
...
const handleFileUpload = (event) => {
  ...
  if (file) { form.bannerFile = file; imagePreview.value = URL.createObjectURL(file); }
}

const removeImage = () => { form.bannerFile = null; form.bannerImage = ''; imagePreview.value = null; }
  ...
  try {
    ...
    if (res.data?.length > 0) {
      ...
      if (!event.winner || event.winner !== event.winnerData.accountID) {
         await api.put(`/api/admin/events/${event.eventID}`, { ...event, winner: event.winnerData.accountID });
      }
    }
  } catch (error) { console.error('Winner fetch failed', error); }
}
...
const openCreateModal = () => {
  isEditing.value = false; isViewOnly.value = false; removeImage();
  Object.assign(form, { 
    eventID: null, eventName: '', bannerFile: null, bannerImage: '',
    description: '', rules: '', reward: '', maxVotes: 3,
    startAt: '', endAt: '', voteStartAt: '', voteEndAt: '',
    originalEndAt: '', originalVoteEndAt: '', winner: null, isActive: 1, isForceEnded: 0 
  })
  showModal.value = true
}

const openEditModal = (ev) => {
  isEditing.value = true; isViewOnly.value = false; removeImage();
  Object.assign(form, { 
    ...ev, 
    startAt: formatForInput(ev.startAt), endAt: formatForInput(ev.endAt), 
    voteStartAt: formatForInput(ev.voteStartAt), voteEndAt: formatForInput(ev.voteEndAt),
    originalEndAt: ev.originalEndAt || ev.endAt, originalVoteEndAt: ev.originalVoteEndAt || ev.voteEndAt 
  })
...
const openViewModal = (ev) => {
  isEditing.value = false; isViewOnly.value = true; removeImage();
  Object.assign(form, { 
    ...ev, 
    startAt: formatForInput(ev.startAt), endAt: formatForInput(ev.endAt), 
    voteStartAt: formatForInput(ev.voteStartAt), voteEndAt: formatForInput(ev.voteEndAt)
...
const executeForceEnd = async () => {
  ...
  try {
    ...
    const payload = { 
      ...ev, 
      originalEndAt: ev.endAt, originalVoteEndAt: ev.voteEndAt,
      endAt: now, voteEndAt: now, isForceEnded: 1 
    }
    await api.put(`/api/admin/events/${ev.eventID}`, payload)
    toast.success(t('admin.events.force_end_ok'))
    showConfirmEndModal.value = false;
    eventToClose.value = null;
    loadEvents() 
  } catch(e) { toast.error(t('admin.events.force_end_fail')) }
}

const viewWinner = async (ev) => {
  showWinnerModal.value = true; loadingWinner.value = true; winnerData.value = null;
  try {
    if(ev.winnerData) { winnerData.value = ev.winnerData; } 
    else {
      const res = await api.get(`/api/admin/events/${ev.eventID}/posts`)
      if (res.data?.length > 0) winnerData.value = res.data.sort((a, b) => b.voteCount - a.voteCount)[0]
    }
  } catch (e) { toast.error(t('admin.events.winner_load_fail')) } finally { loadingWinner.value = false }
}
...
const saveEvent = async () => {
  ...
  try {
    ...
    if (form.bannerFile) finalBannerUrl = await uploadMedia(form.bannerFile, 'events')

    let finalEndAt = form.endAt;
    let finalVoteEndAt = form.voteEndAt;

    if (form.isForceEnded === 0 && form.originalEndAt && new Date(form.originalEndAt) > new Date(form.endAt)) {
        finalEndAt = form.originalEndAt;
        finalVoteEndAt = form.originalVoteEndAt;
    }

    const eventData = { ...form, bannerImage: finalBannerUrl, endAt: finalEndAt, voteEndAt: finalVoteEndAt }
    
    if (isEditing.value) await api.put(`/api/admin/events/${form.eventID}`, eventData)
    else await api.post('/api/admin/events', eventData)

    toast.success(t('admin.events.save_ok'));
    loadEvents();
    showModal.value = false;
  } catch (e) { toast.error(t('admin.common.server_error')); } finally { saving.value = false; }
}
...
const executeDeleteRestore = async () => {
  ...
  try {
    ...
    toast.error(t('admin.common.system_action_error'));
    console.error(e);
  }
}
