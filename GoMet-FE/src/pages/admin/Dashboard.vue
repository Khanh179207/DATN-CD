  ...
  <div class="dashboard-operational">
    ...
    <div class="dash-header animate-fade-down">
      ...
      <button class="btn-refresh" @click="fetchAll" :disabled="loading">
        <RefreshCcw :size="16" :class="{'spinning': loading}" /> {{ $t('admin.common.sync') }}
      </button>
    </div>
    ...
    <div class="bento-grid">
      ...
      <div class="bento-card col-span-2 post-card animate-stagger" style="--i:1">
        ...
        <div class="card-body scrollable">
          <div v-if="loading" class="loading-state"><Loader2 :size="18" class="spinning" style="margin-right: 8px;" /> {{ $t('admin.common.loading_data') }}</div>
          ...
          <div v-for="post in data.posts" :key="post.id" class="list-item-row">
            ...
            <div class="item-actions">
              <button @click="approvePost(post.id)" class="btn-act check" :title="t('admin.posts.action_approve')"><Check :size="16" /></button>
              <button @click="askRejectPost(post)" class="btn-act cross" :title="t('admin.posts.action_reject')"><X :size="16" /></button>
            </div>
          </div>
        </div>
      </div>
          ...
          <div v-for="ap in data.appeals" :key="ap.id" class="report-row">
            <div class="rp-info">
              <div class="rp-top-line">
                <span v-if="ap.statusLabel !== t('admin.dashboard.appeal_unknown')" class="tag-reason" :class="getAppealStatusClass(ap.statusRaw)">{{ ap.statusLabel }}</span>
      ...
      <div v-if="rejectModal.show" class="modal-overlay" @click.self="rejectModal.show = false">
        <div class="action-modal">
          ...
          <div class="action-btns">
            <button class="btn-cancel" @click="rejectModal.show = false">{{ $t('admin.common.cancel') }}</button>
...
import { formatLocaleDate, getDateLocale } from '@/i18n'
import {
...
const formatShortDate = (value, fallbackKey = 'admin.dashboard.recently') => (
  value ? formatLocaleDate(value, locale.value) : t(fallbackKey)
)

const monthFormatter = new Intl.DateTimeFormat(getDateLocale(locale.value), { month: 'short' })
  ...
  try {
    ...
    if (postsRes.status === 'fulfilled') {
      ...
      data.posts = pendings.slice(0, 4).map(p => {
        ...
        const authorName = p.username || p.authorName || p.accountName || p.account?.username || t('admin.posts.anonymous');
        ...
        return {
          ...
          time: formatShortDate(p.createdAt, 'admin.dashboard.just_now'),
    ...
    if (appealsRes.status === 'fulfilled') {
      ...
      data.appeals = allAppeals.slice(0, 4).map(a => ({
        ...
        time: formatShortDate(a.createdAt)
      }));
    ...
    if (commentsRes.status === 'fulfilled') {
      ...
      data.comments = allCmt.slice(0, 4).map(c => {
        const username = c.username || c.authorName || c.accountName || c.account?.username || t('admin.posts.anonymous')
        ...
        return {
          ...
          time: formatShortDate(c.createdAt, 'admin.common.empty_value'),
          avatar: avatar
        }
      })
    }
    ...
    if (eventsRes.status === 'fulfilled') {
      ...
      data.events = (eventsRes.value.data || []).slice(0, 3).map(e => {
        ...
        return {
          ...
          date: start ? { d: String(start.getDate()).padStart(2,'0'), m: monthFormatter.format(start) } : { d: '--', m: '--' },
...
const approvePost = async (id) => {
  ...
  } catch (e) { toast.error(t('admin.dashboard.approve_fail_prefix').replace(/[:：]\s*$/, '')) }
}
...
const confirmRejectPost = async () => {
  ...
  } catch (e) { toast.error(t('admin.dashboard.reject_fail_prefix').replace(/[:：]\s*$/, '')) }
}
