...
onMounted(async () => {
  try {
    ...
    dishes.value = raw.map(dto => ({
      id: dto.postID, name: dto.title, category: dto.categoryName || t('landing.plan_tag'),
      time: dto.cookingTime ? `${dto.cookingTime} ${t('common.minutes')}` : '—',
      difficulty: { 1: t('common.easy'), 2: t('common.medium'), 3: t('common.hard') }[dto.level] || t('common.medium'),
      image: dto.media, description: dto.description || t('leaderboard.default_desc')
    }))
  } catch (e) {}
})
</script>
