        ...
        <div class="sidebar-sticky">
          ...
          <div class="profile-summary premium-card">
            ...
            <div class="user-text">
              ...
              <span class="u-name">{{ authStore.user?.name || $t('common.guest_user') }}</span>
            </div>
          </div>
      ...
      <main class="collection-content">
        ...
        <header class="collection-header premium-card">
          <div class="header-left">
            <div class="brand-tag">{{ $t('storage.brand_tag') }}</div>
            <h1 class="main-title">
      ...
      <div class="popup-box fade-scale">
        ...
        <div class="popup-actions">
          ...
          <button class="btn-delete" @click="confirmUnsave" :disabled="isRemoving">
            {{ isRemoving ? $t('common.loading') : $t('storage.unsave') }}
          </button>
        </div>
      </div>
    </div>
...
const normalizePost = (fav) => {
  return {
    id: fav.postID,
    title: fav.title ?? t('common.no_title'),
    category: fav.categoryName ?? t('common.uncategorized'),
    image: fav.media || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c',
    time: fav.cookingTime ? `${fav.cookingTime} ${t('common.minutes')}` : `30 ${t('common.minutes')}`,
    ...
    author: {
      name: fav.userName ?? t('compare.default_author'),
      avatar: fav.avatar ?? null
    },
    difficulty: getDifficultyLabel(fav.level)
  }
}
...
const confirmUnsave = async () => {
  ...
  try {
    ...
    toast.error(t('toast.remove_post_failed'))
  } finally {
