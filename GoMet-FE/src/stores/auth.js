...
import i18n, { formatLocaleDateTime } from '@/i18n'
...
// 🔥 Force-logout event fired by api.js interceptor on 401/403
  window.addEventListener('auth:force-logout', (event) => {
    ...
    if (isBanned) {
      ...
      const fullMsg = `${i18n.global.t('auth.account_locked')}${timeStr}. ${i18n.global.t('auth.banned_reason')}: ${banReason}.`;
...
async function login(email, password) {
  ...
  return { user, isAuthenticated, isAdmin, currentUser, login, logout, refreshProfile, setUser, resetViews }
})