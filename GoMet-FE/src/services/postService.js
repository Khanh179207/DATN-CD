...
export function normalizePost(dto) {
  if (!dto) return null
  const diffMap = {
    1: i18n.global.t('common.easy'),
    2: i18n.global.t('common.medium'),
    3: i18n.global.t('common.hard'),
  }
  return {
    id:         dto.postID,
    title:      dto.title,
    ...
    time:       dto.cookingTime ? `${dto.cookingTime} ${i18n.global.t('common.minutes')}` : '—',
    difficulty: diffMap[dto.level] || i18n.global.t('common.medium'),
    likes:      dto.favoriteCount ?? 0,
    rating:     dto.avgRating ?? 0,
    views:      dto.views ?? 0,
    authorID:   dto.authorID ?? dto.accountID ?? null,
    author: {
      name:   dto.authorName || i18n.global.t('compare.default_author'),
      avatar: dto.authorAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(dto.authorName || i18n.global.t('common.user_fallback'))}&background=EA580C&color=fff`
    },
