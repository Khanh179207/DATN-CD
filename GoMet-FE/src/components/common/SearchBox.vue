      ...
      <transition name="search-pop">
        <div v-if="showSearchHistory" class="history-card" @mousedown.prevent>
          ...
          <div v-if="isLoadingHistory" class="history-loading">
            ...
            <span>{{ t('search.loading_history') }}</span>
          </div>

          <div v-else-if="errorMessage" class="history-error">
            <span>{{ errorMessage }}</span>
            <button class="btn-retry" @click="loadSearchHistory">{{ t('common.retry') }}</button>
          </div>

          <div v-else-if="suggestions.length > 0">
            <div class="card-header">
              <span class="label">{{ searchKeyword.trim() ? t('search.suggestions_label') : t('search.recent_label') }}</span>
              <span v-if="!searchKeyword.trim()" class="btn-clear-all" @click.stop="clearAllSearchHistory">{{ t('search.clear_all_history') }}</span>
            </div>
          ...
          <div v-else class="history-empty">
            <span>{{ searchKeyword.trim() ? t('search.no_suggestions') : t('search.no_history') }}</span>
          </div>
...
const loadSearchHistory = async () => {
  ...
  try {
    ...
    errorMessage.value = t('toast.load_error')
    searchHistory.value = []
  } finally {
...
const suggestions = computed(() => {
  ...
  if (keyword && !filtered.some(item => item.keyword.toLowerCase() === keyword)) {
    if (filtered.length < 5) {
      filtered.push({ type: 'suggest', id: `suggest-${Date.now()}`, keyword: keyword, displayText: t('search.search_for', { keyword: searchKeyword.value.trim() }) })
    }
  }
  return filtered
})
