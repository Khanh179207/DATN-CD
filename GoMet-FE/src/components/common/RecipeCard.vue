    ...
    <div class="card-media">
      ...
      <div class="quick-actions">
        ...
        <button class="btn-action btn-compare" :class="{ active: compareStore.isSelected(post.id) }" 
          @click.stop="compareStore.toggleItem(post)" :title="$t('common.compare')">
        ...
        <button class="btn-action btn-mealplan" @click.stop="handleSaveToPlan" :title="$t('mealplan.save_success')">
    ...
    <div class="card-content">
      <div class="meta-top">
        <div class="rating-box" :class="{ 'is-new': displayRating === 0 }">
          ...
          <span class="score">{{ displayRating > 0 ? Number(displayRating).toFixed(1) : $t('common.new_label') }}</span>
          <span v-if="displayReviewCount > 0" class="count">({{ displayReviewCount }})</span>
        </div>
        
        <div class="time-badge" :title="$t('recipe.time_label')">
      ...
      <div class="card-footer">
        <div class="author-info">
          <div class="author-avt">
            <img v-if="post.author && post.author.avatar" :src="post.author.avatar" :alt="$t('common.author_avatar')">
            <div v-else class="avt-placeholder">{{ getInitials(post.author?.name) }}</div>
          </div>
          <span class="author-name">{{ post.author?.name || $t('compare.default_author') }}</span>
        </div>

        <div class="card-stats-wrapper">
          
          <div class="stats-view-group" :title="$t('post.views')">
        ...
        <div v-if="showLikesModal" class="likes-popup-overlay" @click.stop="showLikesModal = false">
          <div class="likes-popup-container" @click.stop>
            <div class="popup-header">
              <span>{{ $t('recipe.likes_title') }}</span>
              <button class="btn-close-mini" @click="showLikesModal = false">✕</button>
            </div>
            <div class="popup-scroll-area">
              <div v-if="isLoadingLikesList" class="popup-state">{{ $t('recipe.likes_loading') }}</div>
              <div v-else-if="likesList.length === 0" class="popup-state">{{ $t('recipe.no_likes_yet') }}</div>
...
const formattedTimeAgo = computed(() => {
  ...
  if (!dateStr) return t('common.just_now');
...
const openLikesModal = async () => {
  ...
  try {
    ...
    toast.error(t('toast.load_likes_failed'))
  } finally {
...
const handleSaveToPlan = () => {
  if (!authStore.isAuthenticated) {
    return toast.warn(t('toast.need_login_feature'));
  }
...
const toggleSave = async () => {
  if (!authStore.isAuthenticated) {
    return toast.warn(t('toast.need_login_save_post'));
  }
  ...
  try {
    ...
    toast.error(t('toast.action_failed_check_connection'));
  } finally {
