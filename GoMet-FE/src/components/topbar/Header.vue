<template>
  <header class="content-header" :class="[{ 'is-scrolled': isScrolled }, { 'theme-dark': route.meta?.isDark }]">
    ...
    <div class="header-right">
            ...
            <div v-if="showShopping" class="common-dropdown shopping-width">
              <div class="dropdown-header">
                ...
                <div style="display: flex; gap: 15px;">
                  <span v-if="shoppingStore.items.some(i => i.checked)" class="action-link"
                    @click="shoppingStore.removeCheckedItems()" style="color: #EF4444;">
                    🗑️ Xóa món đã tick
                  </span>
              ...
              <div class="dropdown-body scroll-body">
                ...
                <div v-else v-for="(item, idx) in shoppingStore.items" :key="idx" class="shop-item"
                  :class="{ checked: item.checked }" @click="shoppingStore.toggleItem(idx)">
                  <div class="checkbox-circle">
                    <svg v-if="item.checked" width="12" height="12" viewBox="0 0 24 24" fill="none"
              ...
              <div class="dropdown-footer" v-if="shoppingStore.items.length > 0"
                style="padding: 12px; border-top: 1px solid #F3F4F6;">
      ...
      <div v-if="isDropdownOpen || showNoti || showShopping" class="click-outside-header" @click="closeAllDropdowns">
      </div>
    </div>
...
const openFeedbackModal = () => { closeAllDropdowns(); showFeedbackModal.value = true }
...
.header-avt {
  ...
  /* Cực kỳ quan trọng để ảnh full box */
  background-color: #FFF7ED;
  /* Màu nền nhẹ nếu ảnh đang load */
...
.content-header.theme-dark .btn-icon {
  background: transparent;
  color: #9CA3AF;
}

.content-header.theme-dark .btn-icon:hover {
  background: rgba(255, 255, 255, 0.1);
  color: #FFF;
}

.content-header.theme-dark .divider-vertical {
...
.content-header.theme-dark .name {
  color: #FFF !important;
}

.content-header.theme-dark .list-item {
  border-bottom-color: rgba(255, 255, 255, 0.05) !important;
}

.content-header.theme-dark .empty-state {
...
.content-header.theme-dark .header-info .name {
  color: #FFF !important;
}

.content-header.theme-dark .menu-list li {
  color: #D1D5DB !important;
}

.content-header.theme-dark .menu-list li:hover {
  background: rgba(255, 255, 255, 0.1) !important;
  color: #FFF !important;
}

.content-header.theme-dark .divider {
  background: rgba(255, 255, 255, 0.1) !important;
}

.content-header.theme-dark .admin-link {
  color: #FFF !important;
}
</style>