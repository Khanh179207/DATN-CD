    ...
    <div class="h-left">
      <div class="breadcrumb-zenith">
        ...
        <div class="status-indicator">
          <span class="route-title">{{ route.name || t('admin.topbar.control_center') }}</span>
        </div>
      </div>
    </div>
      ...
      <div class="action-wrap" v-click-outside="closeNoti">
        ...
        <transition name="zenith-drop">
          <div v-if="showNoti" class="panel-zenith noti-panel">
            <div class="z-head">
              <span class="z-title">{{ t('admin.topbar.system_notifications') }}</span>
              ...
              <div v-for="notification in notifications" :key="notification.notificationID" class="z-card"
                ...
                <div class="card-desc">
                  <div class="title-row">
                    ...
                    <span v-if="notification.isRead === 0" class="badge-new">{{ t('common.just_now').toUpperCase() }}</span>
                  </div>
                  <span class="time-txt">{{ formatTimeAgo(notification.createdAt) }}</span>
                </div>
              </div>
            ...
            <div v-else class="z-body custom-scroll empty-state">
              <p style="text-align: center; color: #999; padding: 40px 20px;">
                {{ t('admin.topbar.empty_notifications') }}
              </p>
            </div>
            <div class="z-foot">
              <button @click="$router.push('/admin/notifications')">{{ t('header.see_all_noti') }}</button>
            </div>
          </div>
        </transition>
      </div>
      ...
      <div class="action-wrap" v-click-outside="closeUser">
        <div class="capsule-trigger" @click="toggleUser" :class="{ active: showUser }">
          ...
          <div class="cap-meta">
            ...
            <span class="cap-role">{{ t('admin.topbar.senior_admin') }}</span>
          </div>
          ...
          <div v-if="showUser" class="panel-zenith user-panel">
            ...
            <div class="u-menu-grid">
              <button class="u-menu-item" @click="goToProfile">
                ...
                <span>{{ t('admin.topbar.profile') }}</span>
              </button>
...
const updatePageTitle = () => {
  if (unreadCount.value > 0) {
    document.title = `(${unreadCount.value}) ${t('admin.topbar.page_title')}`
  } else {
    document.title = t('admin.topbar.page_title')
  }
}
...
const handleRealtimeAlert = (event) => {
  ...
  const newNotification = {
    ...
    title: alertData.title || t('admin.topbar.system_alert'),
...
const handleRealtimeAdminNotification = (event) => {
  ...
  const newNotification = {
    ...
    title: notificationData.title || t('admin.topbar.notification'),
...
// --- 👤 USER DATA ---
const adminName = computed(() => auth.currentUser?.username || t('admin.topbar.default_admin_name'))
const adminEmail = computed(() => auth.currentUser?.email || t('admin.topbar.default_admin_email'))
