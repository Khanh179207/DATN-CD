      ...
      <div v-if="isVisible" class="gomet-modal-overlay" @click="closeModal">
        ...
        >
          ...
          <div class="gomet-modal-body custom-scroll-orange">
            <div v-if="!formData.targetPostID" class="user-menu-notice">
              ...
              <div class="notice-text">
                {{ $t('support.report_inline_note') }}
              </div>
            </div>

            <form @submit.prevent="submitFeedback" class="gomet-form-layout">
              <div class="gomet-field">
                <label>{{ $t('support.issue_label') }} <span class="star">*</span></label>
                <div class="select-box-wrap">
                  <select v-model="formData.ticketType" class="gomet-input gomet-select">
                    ...
                    <option value="REPORT" :disabled="!formData.targetPostID">
                      {{ $t('support.type_report') }} {{ !formData.targetPostID ? $t('support.report_only_post') : '' }}
                    </option>
                  </select>
                </div>
              </div>
              ...
              <div v-if="formData.ticketType === 'REPORT' && formData.targetPostID" class="report-info-box">
                <div class="info-content">
                  <div class="info-text">
                    <span class="info-label">{{ $t('support.reporting_post') }}</span>
              ...
              <div class="gomet-field">
                <label>{{ $t('support.title_label') }} <span class="star">*</span></label>
              ...
              <div class="gomet-field">
                <label>{{ $t('support.detail_label') }} <span class="star">*</span></label>
                <textarea
              ...
              <div class="gomet-field">
                <label>{{ $t('support.image_label') }}</label>
                <div class="upload-container">
                  ...
                  <label for="file-support" class="upload-zone" :class="{ 'has-file': filePreview }">
                    <div v-if="!filePreview" class="upload-placeholder">
                      ...
                      <span>{{ $t('support.image_upload') }}</span>
                    </div>
                    <div v-else class="upload-preview-wrap">
                      <img :src="filePreview" class="img-thumb" />
                      <div class="file-meta">
                        <span class="fname">{{ formData.attachment?.name }}</span>
                        <button type="button" @click.stop="removeFile" class="btn-remove-file">{{ $t('support.image_remove') }}</button>
                      </div>
                    </div>
                  </label>
                </div>
          ...
          <div class="gomet-modal-footer">
            <button type="button" class="btn-cancel-text" @click="closeModal">{{ $t('common.close') }}</button>
            <button 
              type="button" 
...
const titleSuggestions = computed(() => {
  if (formData.value.ticketType === 'BUG') {
    return [
      t('support.suggestions.bug.image_load'),
      t('support.suggestions.bug.login_failed'),
      t('support.suggestions.bug.payment_button'),
      t('support.suggestions.bug.page_crash'),
    ]
  } else if (formData.value.ticketType === 'REPORT') {
    return [
      t('support.suggestions.report.offensive_content'),
      t('support.suggestions.report.scam_spam'),
      t('support.suggestions.report.image_copyright'),
      t('support.suggestions.report.illegal_ads'),
    ]
  } else {
    return [
      t('support.suggestions.feedback.new_feature'),
      t('support.suggestions.feedback.home_ui'),
      t('support.suggestions.feedback.hard_to_read'),
    ]
  }
})
...
const handleFileChange = (event) => {
  ...
  if (file) {
    if (file.size > 10 * 1024 * 1024) { toast.error(t('support.file_too_large')); return; }
...
const submitFeedback = async () => {
  if (!formData.value.title.trim() || !formData.value.description.trim()) {
    toast.error(t('support.form_required')); 
    return;
  }
  ...
  try {
    ...
    if (response.data) {
      toast.success(t('support.submitted_ok'));
      closeModal();
    }
  } catch (e) {
