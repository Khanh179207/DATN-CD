    ...
    <div v-if="isOpen" class="modal-overlay" @click.self="closeModal">
      <div class="submit-modal">
        ...
        <div class="modal-header">
          <h2>{{ step === 1 ? $t('event_detail.join_event') : $t('event_detail.choose_recipe') }}</h2>
        ...
        <div v-if="step === 1" class="modal-body step-1">
          <p class="modal-desc">{{ $t('event_detail.choose_submit_method') }}</p>
          ...
          <div class="action-cards">
            <div class="action-card" @click="goToCreatePost">
              ...
              <div class="text-box">
                <h4>{{ $t('event_detail.create_new_recipe') }}</h4>
                <p>{{ $t('event_detail.create_new_recipe_desc') }}</p>
              </div>
              <div class="arrow">
            ...
            <div class="action-card" @click="goToSelectPost">
              ...
              <div class="text-box">
                <h4>{{ $t('event_detail.choose_from_storage') }}</h4>
                <p>{{ $t('event_detail.choose_from_storage_desc') }}</p>
              </div>
              <div class="arrow">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"></path></svg>
              </div>
        ...
        <div v-if="step === 2" class="modal-body step-2">
          <div v-if="isLoading" class="mini-loader">
            ...
            <span>{{ $t('event_detail.loading_posts') }}</span>
          </div>
          ...
          <div v-else-if="myPosts.length > 0" class="my-posts-list">
            <label v-for="p in myPosts" :key="p.postID" class="my-post-item" :class="{ selected: selectedPostId === p.postID }">
              ...
              <img :src="p.media || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=150'" :alt="$t('common.recipe_image')">
              <div class="post-info">
                <h4>{{ p.title }}</h4>
          ...
          <div v-else class="empty-my-posts">
            ...
            <p>{{ $t('event_detail.no_posts_yet') }}</p>
            <button class="btn-outline-dark" @click="goToCreatePost">{{ $t('event_detail.create_now') }}</button>
          </div>
        </div>

        <div v-if="step === 2" class="modal-footer">
          <button class="btn-cancel" @click="step = 1">{{ $t('event_detail.back') }}</button>
          <button class="btn-submit" :disabled="!selectedPostId || isSubmitting" @click="confirmSubmit">
            {{ isSubmitting ? $t('event_detail.submitting') : $t('event_detail.confirm_submit') }}
          </button>
        </div>
...
const goToSelectPost = async () => {
  ...
  try {
    ...
    toast.error(t('event_detail.load_posts_failed'))
  } finally {
...
const confirmSubmit = async () => {
  ...
  try {
    ...
    toast.success(t('event_detail.submit_ok_short'));
    emit('submit-success');
    closeModal();
  } catch (e) {
    toast.error(t('event_detail.submit_fail'));
  } finally {
