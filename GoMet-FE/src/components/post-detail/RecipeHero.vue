    ...
    <div class="hero-container-inner">
      ...
      <div class="hero-info-col">
        ...
        <div class="top-nav-bar">
          ...
          <button class="btn-icon-minimal" @click="openReportModal" :title="$t('common.report')">
        ...
        <div class="recipe-stats-group">
          ...
          <div class="stat-card-clean">
            ...
            <div class="stat-info">
              ...
              <span class="s-val">{{ post.difficulty || $t('compare.default_difficulty') }}</span>
            </div>
          </div>
          ...
          <div class="stat-card-clean highlight">
            ...
            <div class="stat-info">
              <span class="s-label">{{ $t('leaderboard.rating_lbl') }}</span>
              <span class="s-val">{{ post.avgRating > 0 ? Number(post.avgRating).toFixed(1) : $t('common.new_label') }} ({{ post.ratingCount || 0 }})</span>
            </div>
          </div>
        </div>
        ...
        <div class="hero-footer-actions">
          ...
          <div class="author-info-block" @click="router.push(`/profile/${post.authorID}`)">
            <div class="avatar-container">
              <img :src="post.authorAvatar" :alt="$t('common.author_avatar')">
          ...
          <div class="social-actions-row">
            ...
            <div class="action-btn-group">
              ...
              <button class="btn-share-clean" @click="copyLink" :title="$t('common.share')">
      ...
      <div class="hero-image-col">
        <div class="clean-image-frame">
          ...
          <div v-if="isNewToday" class="badge-new-top-left">{{ $t('common.new_badge') }}</div>
            ...
            <div class="modal-body custom-scroll">
              ...
              <ul v-else class="likes-list">
                <li v-for="user in likedUsersList" :key="user.id" class="like-user-item" @click="goToProfile(user.id)">
                  <img :src="user.avatar" :alt="user.name || $t('common.user_avatar')" class="u-avt">
...
// --- COMPUTED LOGIC ---
const displayCategory = computed(() => (props.post.categoryName || props.post.category || t('common.default_recipe_category')).toUpperCase())
const formattedDate = computed(() => {
  const ds = props.post.publishDate || props.post.createdAt;
  return ds ? formatLocaleDate(ds) : formatLocaleDate(new Date());
})
...
// --- ACTIONS ---
const copyLink = () => { navigator.clipboard.writeText(window.location.href); toast.success(t('toast.link_copied')); }
const goToProfile = (id) => { showLikesModal.value = false; router.push(`/profile/${id}`); }
const openReportModal = () => { if (!authStore.isAuthenticated) return toast.warn(t('toast.need_login_report')); showReportModal.value = true; }
  ...
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      likedUsersList.value = response.data.map(u => ({
        ...
        name: u.fullName || u.username || u.name || t('common.user_fallback'),
...
const handleLike = async () => {
  if (!authStore.isAuthenticated) return toast.warn(t('toast.need_login_like_post'));
  ...
  if (isLiked.value) {
    ...
    likedUsersList.value.unshift({
      id: myId,
      name: currentUser.fullName || currentUser.name || t('common.you_label'),
  ...
  try { 
    ...
    if (prevLiked) {
      localLikeCount.value += 1;
      likedUsersList.value.unshift({ id: myId, name: currentUser.fullName || t('common.you_label'), avatar: currentUser.avatar });
    } else {
      localLikeCount.value = Math.max(0, localLikeCount.value - 1);
      likedUsersList.value = likedUsersList.value.filter(u => u.id !== myId);
    }
    toast.error(t('toast.like_action_failed')); 
  } finally { 
...
const toggleFavorite = async () => {
  if (!authStore.isAuthenticated) return toast.warn(t('toast.need_login_save_post'));
  const pid = props.post.postID || props.post.id
  try {
    if (isFavorite.value) { await removeFavorite(authStore.user.accountID, pid); isFavorite.value = false; } 
    else { await addFavorite(authStore.user.accountID, pid); isFavorite.value = true; toast.success(t('toast.added_to_collection')); }
  } catch (e) { toast.error(t('toast.system_error')); }
}
