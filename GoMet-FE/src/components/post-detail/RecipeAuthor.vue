  ...
  <section class="author-section fade-in">
    <div class="author-flat-container">
      ...
      <div class="auth-avatar-col" @click="goToChefProfile" :title="$t('profile.view_profile')">
        <img :src="post.authorAvatar" class="auth-img" :alt="$t('common.author_avatar')">
      </div>
      ...
      <div class="auth-info-col">
        <div class="auth-top-row">
          ...
          <div class="action-buttons">
            <button class="btn-icon-monochrome" :class="{ 'is-saved': isFavorite }" @click="toggleFavorite" :title="isFavorite ? $t('storage.unsave') : $t('post.unsaved')">
            ...
            <button class="btn-icon-monochrome" @click="handleContactChef" :title="$t('common.message')">
            ...
            <button class="btn-follow-black" :class="{ 'is-following': isFollowing }" @click="toggleFollow">
              <span v-if="isFollowing">{{ $t('common.following') }}</span>
              <span v-else>{{ $t('common.follow') }}</span>
            </button>
          </div>
        </div>

        <p class="auth-bio-text">{{ realBio || $t('recipe.empty_bio') }}</p>
        ...
        <div class="auth-stats-inline">
          ...
          <div class="s-item">
            ...
            <span class="s-label">{{ $t('profile.followers') }}</span>
          </div>
          <div class="s-divider"></div>
...
const handleContactChef = async () => {
  ...
  if (currentUserId === chefId) { 
    toast.info(t('toast.self_message_blocked')); return 
  }
  ...
  try {
    ...
    toast.success(t('toast.chat_connecting', { name: props.post.author }))
  } catch (err) { toast.error(t('toast.chat_connect_fail')) }
}
...
const toggleFollow = async () => {
  ...
  } catch (err) { toast.error(t('toast.follow_error')) }
}
...
const toggleFavorite = async () => {
  ...
  } catch (err) { toast.error(t('toast.favorite_error')) }
}
</script>
