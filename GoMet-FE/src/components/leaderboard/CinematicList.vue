<template>
  <section class="cinematic-horizontal-showcase">
    ...
    <div class="cinematic-header">
      <span class="sub-tag">
        {{ category === 'dishes' ? $t('leaderboard.top10_masterpieces') : $t('leaderboard.top10_members') }}
      </span>
        ...
        <div class="item-bottom-info">
          ...
          <div class="item-meta">
            <span class="meta-author" v-if="category === 'dishes'">
              {{ $t('leaderboard.created_by') }} {{ item.authorName || $t('compare.default_author') }}
            </span>
            <span class="meta-author" v-else>
              {{ $t('leaderboard.posts_count', { count: item.postCount || 0 }) }}
            </span>
            ...
            <span class="meta-score">
              ...
              <template v-else>
                {{ $t('leaderboard.followers_count', { count: item.followers || 0 }) }}
              </template>
            </span>
          </div>
        </div>
    ...
    >
      ...
      <div ref="fxPanelRef" class="fx-panel" role="dialog" aria-modal="true">
        <span class="preview-tag">
          {{ category === 'dishes' ? $t('leaderboard.preview_masterpiece') : $t('leaderboard.preview_member') }}
        </span>
        <h3 class="preview-title">{{ previewTitle }}</h3>

        <p class="preview-meta">
          <template v-if="category === 'dishes'">
            {{ $t('leaderboard.created_by') }} {{ previewItem.authorName || $t('compare.default_author') }}
            · {{ previewItem.pts || 0 }} PTS
          </template>
          <template v-else>
            {{ $t('leaderboard.posts_lbl2') }}: {{ previewItem.postCount || 0 }}
            · {{ $t('leaderboard.followers') }}: {{ previewItem.followers || 0 }}
          </template>
        </p>
        ...
        <button class="preview-cta" @click="goToDetail">
          {{ category === 'dishes' ? $t('leaderboard.view_recipe') : $t('common.view_profile') }}
        </button>
      </div>
    </div>
  </section>
