  ...
  <transition name="slide-up">
    <div v-if="chatStore.activeChat" class="mini-chat-box" :class="{ 'is-minimized': isMinimized }">
      ...
      <div class="chat-head" @click="isMinimized = !isMinimized">
        <div class="user-info">
          <div class="avt-wrap">
            <img :src="chatStore.activeChat.avatar || 'https://ui-avatars.com/api/?name=User&background=random'" :alt="$t('common.user_avatar')">
            <span class="dot-status" :class="{ offline: !chatStore.activeChat.online }"></span>
          </div>
          <div class="name-col">
            <h4>{{ chatStore.activeChat.name }}</h4>
            <span class="status-text">{{ chatStore.activeChat.online ? $t('chat.active_now') : $t('chat.active_recently') }}</span>
          </div>
        </div>
        <div class="actions">
      ...
      <div v-show="!isMinimized" class="chat-body-wrapper">
        ...
        <div class="chat-footer">
          <div class="input-actions">
            <button class="btn-action-icon" :title="$t('common.emoji')"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><path d="M8 14s1.5 2 4 2 4-2 4-2"></path><line x1="9" y1="9" x2="9.01" y2="9"></line><line x1="15" y1="9" x2="15.01" y2="9"></line></svg></button>
            <button class="btn-action-icon" :title="$t('common.upload_image')"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg></button>
          </div>
          ...
          <button class="btn-send" @click="sendMsg" :disabled="!inputMsg.trim()" :title="$t('common.send')">
...
const formatTime = (dateInput) => {
  if (!dateInput) return t('recipe.just_posted')
