    ...
    <div class="chat-sidebar" v-if="chatStore.isMessengerOpen" ref="sidebarRef">
      ...
      <div class="sidebar-body scroll-body">
        ...
        <div v-else-if="filteredConversations.length === 0" class="empty-state">
          ...
          <p v-if="searchQuery">{{ $t('chat.no_results') }}: "{{ searchQuery }}"</p>
          <p v-else>{{ $t('chat.no_conversations') }}</p>
        </div>
        ...
        <div v-else class="chat-list">
          ...
          >
            <div class="avatar-wrap">
              <img :src="c.avatar || 'https://ui-avatars.com/api/?name=User&background=random'" :alt="$t('common.user_avatar')">
...
const loadConversations = async () => {
  ...
  try {
    ...
    conversations.value = res.data.map(item => ({
      ...
      time: item.time || t('common.just_now'),
      read: item.read === true,
